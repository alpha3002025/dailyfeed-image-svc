...
```bash
## 수정하려는 helm app 이 dailyfeed-image 일 경우
cd dailyfeed-app-helm
cd image
source uninstall-helm.sh ## dailyfeed-image 제거
source install-helm-local.sh ## dailyfeed-image 실행 
```
...
```bash
# 새로운 환경에서 클론 (clone)
git clone --recurse-submodules http://github.com/alpha3002025/dailyfeed-image-svc  

# 또는 클론 후 submodule 초기화
git clone dailyfeed-image-svc
cd dailyfeed-image-svc
git submodule init
